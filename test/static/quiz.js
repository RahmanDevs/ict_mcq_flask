...
function updateDots() {
  ...
  dots.forEach((d, i) => {
    ...
    if (i === current) {
      ...
      d.classList.add(userAnswers[i] === questions[i].ans ? 'answered-correct' : 'answered-wrong');
    } else if (answered[i]) {
...
function renderQuestion(idx) {
  ...
  q.opts.forEach((opt, i) => {
    ...
    const click = isLocked ? '' : 'onclick="selectOption(' + idx + ',' + i + ',this)"';
    html += '<label class="' + cls + '" ' + click + '>'
          + '<input type="radio" name="q' + idx + '" ' + (isSel ? 'checked' : '') + ' ' + (isLocked ? 'disabled' : '') + '>'
  ...
  wrap.innerHTML = '<div class="question-wrap">'
    + '<div class="' + boxClass + '"><div class="question-title">'
...
function finishQuiz()  { quizSubmitted = true; document.getElementById('overlay').style.display = 'none'; clearInterval(timerInterval); showResults(); }
